 ...
 * ----------------------------------------------
 * The mic stage consumes 15,625 frames/s in 512-frame blocks, one every
 * 32.77 ms. Measured on this Mac against this board:
 *
 ...
 *   JLinkGDBServer RSP 'X' block write   ~36,000 bytes/s  — works
 *
 ...
 * reboots the pendant mid-conversation. The GDB server's binary 'X' packet
 * does true block writes against a RUNNING target (verified: `ishalted` stays
 * "CPU is not halted" throughout, and the conversation keeps streaming).
 *
 * A raw SWD block write tops out at ~146 kB/s here and does not improve with
 * -speed beyond 4000 (it is the J-Link OB USB path, not the SWD clock), so
 * ~36 kB/s through the GDB server is the real budget. At 16-bit PCM that is
 * 36,000 against 31,250 B/s — 15% margin, which is no margin at a 32.77 ms
 * cadence. So the wire format is 8-bit G.711 mu-law: 15,625 B/s (2.3x
 * headroom) and, for the same fixed 4 KB of target RAM, 262 ms of runway
 * instead of 131 ms. The firmware expands it with the mu-law table it already
 * had. The mapping is deterministic, so repeatability is exact.
 *
 * Every write is read back and verified. (There is no `timeout` command on
 * this Mac — a previous session lost an hour to `timeout 25 JLinkExe ...`
 ...
 *   --elf <path>        ELF to resolve symbols from (default: build-cloud)
 *   --keep              keep the rendered PCM next to the output for diffing
...
const DEFAULT_ELF = path.join(
  REPO, 'firmware/nrf9160/build-cloud/nrf9160/zephyr/zephyr.elf');
...
function parseArgs(argv) {
  const o = {
    ...
    leadMs: 300, replyMs: 20000, elf: DEFAULT_ELF, selftest: false,
    keep: false, out: path.join(REPO, 'diagnostics/inject'),
  };
...
// ------------------------------------------------------------------- G.711
// Standard (Sun) mu-law. The decoder below is a transliteration of the
// firmware's ulaw_to_linear(), so `expand(compress(x))` on the host is exactly
// what the pendant will feed its encoder — which lets us write the injected
// waveform to disk and know it is the truth, not an approximation of it.
...
class RspLink {
  ...
  async readMem(addr, len) {
    ...
    for (let off = 0; off < len; off += CHUNK) {
      ...
      const r = await this.cmd(`m${(addr + off).toString(16)},${n}`);
  ...
  async writeU32(addr, value, label) {
    ...
    const r = await this.cmd(
      `X${addr.toString(16)},4:` + escapeBin(b).toString('binary'));
...
function toUlaw(pcm, leadFrames) {
  const silence = ulawCompress(0);
  const bytes = Buffer.alloc(leadFrames + pcm.length);
  bytes.fill(silence, 0, leadFrames);
  for (let i = 0; i < pcm.length; i++) {
    bytes[leadFrames + i] = ulawCompress(pcm[i]);
  }
  return bytes;
}

/* Exactly what the device will decode — the golden reference for a run. */
...
/* Push as much as the ring will hold, honouring the wrap. Returns new head. */
async function refill(link, sym, ulaw, cursor, head, tail) {
  const used = (head - tail) & (RING_FRAMES - 1);
  let free = RING_FRAMES - 1 - used;              // one slot kept so != empty
  let wrote = 0;
  while (free > 0 && cursor + wrote < ulaw.length) {
    const contiguous = Math.min(free, RING_FRAMES - ((head + wrote) & (RING_FRAMES - 1)));
    const n = Math.min(contiguous, ulaw.length - (cursor + wrote));
    if (n <= 0) break;
    const at = (head + wrote) & (RING_FRAMES - 1);
    await link.writeBlock(sym.pendant_inject_ring + at,
                          ulaw.subarray(cursor + wrote, cursor + wrote + n));
    wrote += n;
    free -= n;
  }
  if (wrote > 0) {
    const newHead = (head + wrote) & (RING_FRAMES - 1);
    const b = Buffer.alloc(4); b.writeUInt32LE(newHead, 0);
    await link.writeBlock(sym.pendant_inject_head, b);
    return { head: newHead, cursor: cursor + wrote, wrote };
  }
  return { head, cursor, wrote: 0 };
}
...
async function main() {
  ...
  try {
    ...
    const ulaw = toUlaw(pcm, leadFrames);
    const golden = expandedPcm(ulaw);
    writeWav(path.join(workDir, 'injected.wav'), golden);
    fs.writeFileSync(path.join(workDir, 'injected.ulaw'), ulaw);
    const durS = ulaw.length / SAMPLE_RATE;
    console.log(`  ${ulaw.length} mu-law frames = ${durS.toFixed(2)} s ` +
                `(${(ulaw.length / durS / 1024).toFixed(1)} kB/s on the wire)`);
    console.log(`  golden PCM: ${path.join(workDir, 'injected.wav')}`);

    await armAndReset(link, sym);

    // ---- prefill so stage 1 is never starved
    let head = 0; let cursor = 0;
    ({ head, cursor } = await refill(link, sym, ulaw, cursor, head, 0));
    ...
    const back = await link.readMem(sym.pendant_inject_ring, Math.min(cursor, 256));
    if (!back.equals(ulaw.subarray(0, back.length))) {
      throw new Error('ring prefill verification failed — bytes did not land');
    }
    console.log(`  verified first ${back.length} ring bytes match the source`);
    ...
    // ---- stream
    const stats = await streamLoop(link, sym, ulaw, cursor, head, opts);
    ...
    const summary = {
      ...
      ulawFrames: ulaw.length,
      durationS: Number(durS.toFixed(3)),
      ulawSha256: sha256(ulaw),
...
async function streamLoop(link, sym, ulaw, cursor0, head0, opts) {
  let cursor = cursor0;
  let head = head0;
  let started = false;
  let startedAt = 0;
  ...
  for (;;) {
    ...
    if (started) minRunwayFrames = Math.min(minRunwayFrames, used);

    // Refill when the ring is at least half empty: at 15,625 frames/s that
    // leaves ~131 ms to finish a block write measured at ~57 ms.
    if (cursor < ulaw.length && used <= RING_FRAMES / 2) {
      const t = Date.now();
      const r = await refill(link, sym, ulaw, cursor, head, tail);
      const ms = Date.now() - t;
      if (r.wrote > 0) {
        refills++;
    ...
    await sleep(started ? STAGE_MS / 2 : 200);
  }
...
async function selftest(link, sym) {
  ...
  const pattern = Buffer.alloc(RING_FRAMES);
  for (let i = 0; i < pattern.length; i++) pattern[i] = (i * 97 + 13) & 0xff;

  const t0 = Date.now();
  const reps = 8;
  for (let i = 0; i < reps; i++) {
    await link.writeBlock(sym.pendant_inject_ring, pattern);
  }
  const ms = (Date.now() - t0) / reps;
  const bps = Math.round(pattern.length / (ms / 1000));
  console.log(`  ${pattern.length} B block write: ${ms.toFixed(1)} ms ` +
              `=> ${bps} B/s (need ${SAMPLE_RATE} B/s)`);

  const back = await link.readMem(sym.pendant_inject_ring, RING_FRAMES);
  console.log(`  readback ${back.equals(pattern) ? 'MATCHES' : 'DIFFERS'} ` +
              `over all ${RING_FRAMES} bytes`);
  if (!back.equals(pattern)) throw new Error('block write verification failed');

  // Proof the CPU never halted: a free-running kernel counter must advance.
  const u1 = await link.readU32(sym.pendant_remote_press);
  console.log(`  remote_press reads ${hex32(u1)} (expect 00000000 when idle)`);
  console.log(`  headroom: ${(bps / SAMPLE_RATE).toFixed(2)}x the ` +
              `${SAMPLE_RATE} B/s the mic stage consumes`);
