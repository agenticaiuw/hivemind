...
async function installAgentApp() {
  ...
  /*
   * The one product icon — assets/icon/pendant.icns, generated by
   * scripts/sync-icons.mjs from the single master. This app shipped faceless
   * for months (a bare node wrapper), which is how the product ended up with
   * four different icons at once. Copied best-effort: an installer that
   * refuses to install over a missing icon has its priorities backwards.
   */
  /* REPO_ROOT is the PACKAGE root (software/ai-pendant-simulator) — the
   * launchd working directory — not the git root. The icon assets live two
   * levels up, and the first version of this line missed for exactly that
   * reason: the app installed faceless while claiming success. */
  const icnsSource = path.resolve(REPO_ROOT, '../../assets/icon/pendant.icns')
  if (fs.existsSync(icnsSource)) {
    fs.copyFileSync(icnsSource, path.join(resources, 'AppIcon.icns'))
  }
