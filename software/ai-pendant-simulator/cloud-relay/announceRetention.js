 ...
 *
 * GET reports and changes nothing. POST is dryRun unless the caller sends
 * {"dryRun": false} AND the deployment has not switched retention off.
 */
export function registerAnnouncementRetentionRoutes(app, { getStore } = {}) {
  if (!app || typeof app.get !== 'function' || typeof app.post !== 'function') {
    throw new Error('registerAnnouncementRetentionRoutes requires an Express-style app.')
  }
  if (typeof getStore !== 'function') {
    throw new Error('registerAnnouncementRetentionRoutes requires a getStore().')
  }

  app.get('/v1/ops/announcement-retention', async (request, response) => {
    const store = await getStore()
    const now = Date.now()
    const graceMs = normalizeGraceMs(
      request.query?.graceMs ?? ANNOUNCEMENT_RETENTION_GRACE_MS,
    )
    ...
    response.json({ ok: true, ...report, observedAt: new Date(now).toISOString() })
  })

  app.post('/v1/ops/announcement-retention/sweep', async (request, response) => {
    const store = await getStore()
    const now = Date.now()
    const graceMs = normalizeGraceMs(
      request.body?.graceMs ?? ANNOUNCEMENT_RETENTION_GRACE_MS,
    )
    const requestedDryRun = request.body?.dryRun !== false
    const dryRun = requestedDryRun || !ANNOUNCEMENT_RETENTION_SWEEP_ENABLED
    ...
    response.json({
      ...
      ...report,
      requestedDryRun,
      blockedBySafetyFlag: !requestedDryRun && !ANNOUNCEMENT_RETENTION_SWEEP_ENABLED,
